...
class UserController extends Controller
{
	...
	{
		return AppHelper::send_Push("sddsdsd","https://marketpluss.com",'vendor',7,"aajaj");
	}
...
public function fetch_top_category()
{
  ...
  return response()->json(['status'=>true,'data'=>$categories]);
	...
	public function search_all(Request $request)
	{
		...
		{
			$search_vendor=Vendor::where('status','Active')->where('shop_name','like', '%' . $q . '%')->paginate(10);
			$response['vendor']=$search_vendor;
		}
		else if($request->search_type == 'payment')
		{
			$search_vendor=Vendor::where('status','Active')->where('shop_name','like', '%' . $q . '%')->where('payment_accept',1)->paginate(10);
			$response['vendor']=$search_vendor;
		}
		else{
		//	return $request->vendor_id;
			$search_product=Vendor_Product::with('variants')->with('addon_map')->where('product_name','like', '%' . $q . '%')->where('vendor_id',$request->vendor_id)->paginate(10);
...
//			$response['vendor']=$search_vendor;
    ...
    {
        $user_id=Auth::user()->id;
        $user=User::addSelect(['cashback' =>UserOrders::selectRaw('sum(order_discount)')->where('order_status', 'completed')->where('user_id',$user_id)])->
